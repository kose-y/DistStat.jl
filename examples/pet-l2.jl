...
mutable struct PETVariables_l2{T,A}
    ...
    function PETVariables_l2(y::AbstractArray{T}, E::MPIMatrix{T,A}, 
        ...
        new{T,A}(m, n, y, E, mu, eps, G, D, N, a, lambda, lambda_prev, 
            tmp_mn, tmp_n1, tmp_n2, tmp_m_local)
    end
end
...
function update!(u::PETUpdate_l2, v::PETVariables_l2)
    ...
    v.tmp_mn .= v.E .* v.y .* transpose(v.lambda) ./ (el .+ v.eps) # z, 
    ...
    v.tmp_n2 .= reshape(sum(z; dims=1), :) # c.
    if v.mu != 0
...
v = PETVariables_l2{A}(y, E, G, D, mu; eval_obj=eval_obj)
pet_l2!(uquick, v)
reset!(v)
if DistStat.Rank() == 0
    @time pet_l2!(u, v)
