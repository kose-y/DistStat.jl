...
for T in type
    ...
    r_local = Array{T}(undef, N, M)
    LinearAlgebra.mul!(r_local, X_dist, XX)
    @test isapprox(r_local, result3)

    LinearAlgebra.mul!(transpose(r_local), transpose(XX), transpose(X_dist))
    @test isapprox(r_local, result3)
